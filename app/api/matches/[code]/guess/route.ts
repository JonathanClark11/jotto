...
export async function POST(request: Request, context: { params: Promise<{ code: string }> }) {
  ...
  const duplicate = await db.prepare(
    "SELECT id FROM cinq_guesses WHERE match_code = ? AND player = ? AND word = ?",
  ...
  if (match.current_turn !== role) {
    await db.prepare(`INSERT INTO cinq_pending_guesses (match_code, player, word, created_at)
      VALUES (?, ?, ?, ?)
  ...
  const turnRow = await db.prepare(
    "SELECT COALESCE(MAX(turn_number), 0) AS last_turn FROM cinq_guesses WHERE match_code = ?",
  ...
  const statements = [
    db.prepare(`INSERT INTO cinq_guesses
      (match_code, player, turn_number, word, match_count, created_at)
      SELECT ?, ?, ?, ?, ?, ?
      WHERE EXISTS (
        SELECT 1 FROM cinq_matches
  ...
  statements.push(
    db.prepare(`UPDATE cinq_matches
      SET status = ?, current_turn = ?, winner = ?, updated_at = ?
      WHERE code = ? AND status = 'active' AND current_turn = ?`)
      .bind(finalWinner ? "finished" : "active", nextTurn, finalWinner, now, code, role),
    db.prepare("DELETE FROM cinq_pending_guesses WHERE match_code = ? AND player = ?")
      .bind(code, role),
    db.prepare("DELETE FROM cinq_pending_guesses WHERE match_code = ? AND player = ?")
      .bind(code, opponent),
  );
  if (finalWinner) {
    statements.push(db.prepare("DELETE FROM cinq_pending_guesses WHERE match_code = ?").bind(code));
  }
